...
const AttractionBookingForm = (props) => {
  ...
  const selectedBurjTimeSlot = async (timeslot) => {
    setticketPrice(null);
    const postObject = {
      agencyId: loginData?.data?.userId ? loginData?.data?.userId : 0,
      agencyUserId: 0,
      bookB2cId: 0,
  ...
  const getTicketPrice = async (event) => {
    ...
    try {
      const price = await fetchPriceforAgents({
        value: value,
        eventtypeId: eventtypeId,
        attId: attId,
    ...
    @media (min-width: 1024px) {
     ...
     async function postApi(leadPassenger, payload) {
        ...
        const postObject = {
          ...
          "nofAdult": 1,
          "nofChild": 0,
          "bookAdultPrice":Number(formData.nofAdult),
          "bookChildPrice": Number(formData.nofChild),
